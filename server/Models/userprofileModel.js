...
Profile.deleterequest = async (requestID) => {
  ...
  Profile.mysittersapplications = async (userID) => {
    try {
      const result = await db.query(
        `select sitter_applications.id,users.user_name,sitter_applications.childern_count
        REPLACE(users.image, 'https://storage.googleapis.com/sittersphere-bfd8b.appspot.com/BabySitters/', '') AS image,
        ...
        ,sitter_applications.salary,sitter_applications.period from sitter_applications where sitter_applications.user_id = $1`,
        [userID]
      );
      const formattedResult = await Promise.all(
        result.rows.map(async (row) => {
